...
EOF
    ...
    echo "dwc_otg.lpm_enable=0 root=$PARTUUID rootfstype=ext4 elevator=deadline rootwait init=/sbin/init.resizefs ro cgroup_memory=1 cgroup_enable=memory" | sudo tee boot/cmdline.txt >/dev/null
    sudo rm -rf $PITEMP
    elif [ "$IMAGE_TYPE" = "orangepipc2" ]; then
